...
def save_zonearray(zonedist,VG,ws):
    ...
    np.save(os.path.join(ws, 'zonearray.npy'), zonearray)


def Init_Modflow(ws, name,gridprops,radius, GI, k,D,c, Q,TBC, ObsR, ss=0.001, nlay =1, Tlen = 1, Tsteps = 240,Tmult = 1.1):
    ...
    if nlay == 1:
        ...
        npf = flopy.mf6.ModflowGwfnpf(gwf,k=[k[0]/24, (D/c)/24,k[1]/24],save_specific_discharge=True)
    ...
    oc = flopy.mf6.ModflowGwfoc(gwf,budget_filerecord=f"{name}.bud",head_filerecord=f"{name}.hds",
        saverecord=[("HEAD", "ALL"), ("BUDGET", "ALL")],  printrecord=[("HEAD", "LAST")])
...
def Set_Obs(gwf, nlay,GI, ObsR):
    cont = {}
    for o in ObsR:
        obscell = GI.intersect(Point([0,o]))[0][0]
        for n in range(nlay):
            cont[f'head_{n}_{round(o,0)}'] = ['head', 'head', (n,obscell)]
